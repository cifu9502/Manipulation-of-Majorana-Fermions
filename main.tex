...
% ---------------------------------------------------
\subsection{Methods}
For the non-interacting case $(U=0)$, we use the ballistic transport approach \cite{} to compute the Green functions associated to both quantum dot operators $\Green{d_id^\dagger_i}$. 
\begin{equation}
  \Green{A,B}= \mathcal{F}\left\{ \mathcal{T}\left[\left\{ A(t),B(t')\right\} \right]\right\} \left(\omega\right),
  \label{eq:greenFunction}
\end{equation}
\noindent with $\mathcal{F}$ denoting the Fourier transform. The density of states is 
\begin{equation}
