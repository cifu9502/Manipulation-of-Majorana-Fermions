...
%-----------E N D  F I G U R E  5 ------
     ...
     % Figure \ref{fig:t1=t2}.
     
     % The density of states for setup (a) in FIG.\ref{fig:MajoranaModels}) is shown in Figure \ref{fig:t1=t2}. S
...
% <<<<<<< HEA
    ...
    The results of the third configuration FIG.\ref{fig:MajoranaModels}(c) appear in FIG.\ref{fig:t2>0}. With the MZM is coupled only to the second dot, it is quite impressive that a Majorana signature of height $\frac{0.5}{\pi \Gamma}$ appears it the first dot since despite is no direct connection  between them. Contrary to what was observed in the previous case, this time the Majorana signature is not destroyed by the interference but instead, it emerges indirectly in the first dot. This is a perfect way to separate the Majorana the spin-$\up$ DOS from the central spin-$\up$ zero-mode which is still destroyed by the interference. In addition, the second dot still exhibits a Majorana signature as observed in FIG.\ref{fig:t2>0}(b). Turning on the gate voltage in dot $1$ destroys the Majorana signature in both dots FIG.\ref{fig:t2>0}(c)(d). However, if the gate voltage in dot $2$ is turned on, the Majorana mode will leave QD2 but the indirect majorana signature will prevail in QD1. 
