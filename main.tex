...
% This effective model is able to reproduce the physical effects of more elaborated couplings \cite{ruiz-tijerina_interaction_2015} such 
...
%-----------E N D  F I G U R E  5 ------
    ...
    The results of the third configuration FIG.\ref{fig:MajoranaModels}(c) appear in FIG.\ref{fig:t2>0}. Contrary to what was observed in the previous case, this time the Majorana signature is not destroyed by the interference but instead, the  $\frac{0.5}{\pi \Gamma}$-height MZM emerges indirectly in the first dot. This is a perfect way to separate the Majorana's spin-$\dw$ DOS from the central spin-$\up$ zero-mode which is still destroyed by the interference. In addition, the second dot still exhibits a Majorana signature as observed in FIG.\ref{fig:t2>0}(b). In the second row we observe that turning on the gate voltage in dot $1$  destroys the Majorana signature in both dots FIG.\ref{fig:t2>0}(c)(d). On the other hand, if the second dot's voltage is switched on the Majorana signature will still appear in both dots, while the spin-$\up$ quantum interference vanishes. 


 
    In total we were able to distinguish two types of Majorana signatures:
     \begin{itemize}
...
\end{figure}
    ...
    The energy regime we are interested in this project is the one observed in the inset of FIG.\ref{fig:NRG_Majorana}  close to the Kondo temperature and the emergence of Majorana physics. FIG.\ref{fig:Nt1=t2} shows the NRG results symmetric setup in FIG.\ref{fig:MajoranaModels}(a). In agreement with the non-interacting results, we observe that the Majorana signature, which is initially localized in both dots, can be manipulated by tuning the QD gate voltages. 



     % which are closed to the Kondo temperature $T_KK_B \sim \Gamma_1 \frac{\Gamma_1}{U}$, which also coincides with the regime where the Majorana effects are visible  $T_MK_B \sim \frac{t^2}{U}$.  
